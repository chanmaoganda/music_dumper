use std::io::{Read, Seek};

const KEY: [u8; 256] = [
    ,
];

#[test]
fn zero_check() -> anyhow::Result<()> {
    let mut reader = std::fs::File::open("..\\target\\qmc_test\\first_demo.mgg")?;
    let mut buffer = Vec::new();
    let size = reader.read_to_end(&mut buffer)?;
    let mut cursor = std::io::Cursor::new(buffer);
    cursor.seek(std::io::SeekFrom::Start(size as u64 - 7))?;
    let mut last_four_bytes = Vec::new();
    let size = cursor.read_to_end(&mut last_four_bytes)?;
    assert_eq!(size, 7);
    assert_eq!(String::from_utf8(last_four_bytes)?, "QTag".to_owned());	
    Ok(())
}